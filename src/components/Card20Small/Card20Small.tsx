...
import PostCardMeta from "components/PostCardMeta/PostCardMetav3";
import { PostDataType } from "data/types";
import { Link } from "react-router-dom";

export interface Card20SmallProps {
  className?: string;
  post: PostDataType;
}

const Card20Small: FC<Card20SmallProps> = ({ className = "h-full", post }) => {
  ...
  return (
    ...
    >
      ...
      <Link to={href} title={title} className={`block sm:w-20 flex-shrink-0 relative rounded-lg overflow-hidden mb-5 sm:ml-4 sm:mb-0 group`}>
