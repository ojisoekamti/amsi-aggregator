...
const postsDemo1: PostDataType[] = DEMO_POSTS_NEWS.filter((_: any, i: number) => i > 0 && i < 6);
const postsDemo2: PostDataType[] = DEMO_POSTS_NEWS.filter((_: any, i: number) => i > 5 && i < 11);
const postsDemo3: PostDataType[] = DEMO_POSTS_NEWS.filter((_: any, i: number) => i > 11 && i < 17);
...
const SectionMagazine11: FC<SectionMagazine11Props> = ({
  ...
  const renderListByCat = (category: TaxonomyType, listPosts: PostDataType[]) => {
    return (
      <div key={category.id} className={`flex flex-col space-y-4`}>
        <div className="flex items-center justify-between">
          <Badge className="uppercase tracking-wide rounded-none px-4 py-1.5" name={category.name} color={category.color as TwMainColor} />
          <a href="##" className="flex items-center text-xs text-neutral-500">
            <span>More Articles</span>
            <i className="ml-1.5 las la-arrow-right"></i>
          </a>
        </div>
        {posts[0] && <Card18 ratio="aspect-w-4 aspect-h-3" className="flex-shrink-0" post={listPosts[0]} />}
            ...
            .map((post) => (
              <li key={post.id}>
                <h2 className="nc-card-title flex items-start font-semibold space-x-4">
                  <Badge className="w-2.5 h-2.5 !p-0 rounded flex-shrink-0 mt-2" name={""} color={category.color as TwMainColor} />
  ...
  return (
    <div className={`nc-SectionMagazine11 relative ${className}`}>
      <Heading desc={"Click on music icon and enjoy music or podcast"}>Listen to audio live</Heading>
